...
class TestPlayerActionStats(unittest.TestCase):
    ...
    def test_table_exists_with_expected_columns(self):
        ...
        self.assertEqual(
            cols,
            {
                ...
                "action_text", "consequence_kind", "hull_integrity", "created_at",
            },
        )
    ...
    def test_save_round_trips_all_fields(self):
        ...
        self.assertEqual(result["hull_integrity"], 84)
        ...
        self.assertEqual(row["hull_integrity"], 84)
    ...
    def test_same_player_in_two_games_does_not_mix(self):
        ...
        db.save_player_action_stats(
            ...
            action_id="action_1", action_text="a", consequence_kind="progress", hull_integrity=100,
        )
        db.save_player_action_stats(
            game_id="wjptt8", player_id=222, turn=1,
            action_id="action_2", action_text="b", consequence_kind="fatal", hull_integrity=95,
        )
...
class TestBriefingSchemaConsequenceKind(unittest.TestCase):
    ...
    def _choice_item_schema(self):
        # Bypass __init__ (which constructs an OpenAI client); the schema only
        # needs the action-quota attributes.
        from game_server import GameServer

        dummy = GameServer.__new__(GameServer)
        dummy.turn_progress_actions = 1
        dummy.turn_injury_actions = 1
        dummy.turn_fatal_actions = 1
        schema = dummy._get_player_briefing_schema()
    ...
    def test_consequence_kind_is_enum(self):
        ...
        # 'delay' is the system hesitation fallback — valid in the enum so
        # stored choices parse, but the LLM is told never to emit it.
        self.assertEqual(set(prop["enum"]), {"progress", "injury", "fatal", "delay"})
