...
def build_scene_instruction_user(
    ...
    if language == LANGUAGE_RU:
        return (
            ...
            f"{anatomy_guard_ru}"
            "Важно о локации: каждый член экипажа находится ВНУТРИ корабля на своём посту "
            "(рубка, инженерный отсек, медицинский отсек, лаборатория и т.д.). "
            "Текст действия часто описывает события СНАРУЖИ корабля — гравитационный шторм, "
            "обломки, орбита, радиация, маневровые двигатели, щиты. Это то, КУДА НАПРАВЛЕНО "
            "действие, а НЕ то, где стоит персонаж. НЕ помещай персонажа в космос из-за таких слов. "
            "Выбирай exterior_ship или planet_surface ТОЛЬКО если в действии явно сказано, что "
            "персонаж в скафандре, шлюзе, шаттле, высадке на поверхность или физически вне корабля. "
            "Во всех остальных случаях выбирай внутренний отсек (bridge по умолчанию для приказов/управления, "
            "engineering для ремонта/двигателей/энергосистем, sickbay для лечения, lab для исследований). "
    ...
    return (
        ...
        f"{anatomy_guard_en}"
        "Important about location: every crew member is INSIDE the ship at their station "
        "(bridge, engineering bay, sickbay, lab, etc.). The action text often describes events "
        "OUTSIDE the ship — gravitational storm, debris, orbit, radiation, maneuvering thrusters, "
        "shields. That is WHERE THE ACTION IS DIRECTED, NOT where the character stands. Do NOT place "
        "the character in space because of those words. Only pick exterior_ship or planet_surface if "
        "the action explicitly states the character is in a spacesuit, airlock, shuttle, surface landing, "
        "or otherwise physically off the ship. Otherwise pick an interior compartment (bridge by default "
        "for command/controls, engineering for repairs/engines/power systems, sickbay for healing, "
        "lab for research). "
