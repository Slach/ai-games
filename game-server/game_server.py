...
class GameServer:
    ...
    async def _call_llm(
        ...
        *,
        ...
        LLM parameters (temperature, max_tokens, enable_thinking) are resolved
        from :mod:`llm_config` via ``use_case`` and the instance's VS flag.
        ...
        Args:
            use_case: Key into :data:`llm_config.USE_CASES`; selects the
                vs/non_vs parameter branch based on ``self.vs_enabled``.
        ...
        params = resolve_llm_params(self.llm_model, use_case, self.vs_enabled)
    ...
    async def _call_llm_text(self, system_prompt: str, user_prompt: str) -> str:
        ...
        try:
            params = resolve_llm_params(self.llm_model, "player_message_text", self.vs_enabled)
            response = await self.client.chat.completions.create(
                model=self.llm_model,
